...
# CHI UPDATED TIME FRAME - TIME SERIES LINE GRAPH
plot_variable <- function(var) {
  ...
  # Data with no NAs for continuous lines where possible
  data_no_nas <- data %>% filter(!is.na(value))
...
```{r}
# CHI: BAR GRAPH W/ AVERAGE CHANGE
...
#   # Create the bar plot
#   ggplot(aggregated_data, aes(x = fiscal_year, y = mean_value, fill = quantile_group_labels)) +
#     geom_col(position = "dodge") +
#     scale_x_continuous(breaks = seq(2012, 2022, by = 1), limits = c(2012, 2022)) +
#     scale_y_continuous(labels = scales::comma) +
#     viridis::scale_fill_viridis(discrete = TRUE, option = "mako", end = .94) +
#     theme_bw() +
#     labs(title = paste0("Average Change in ", var, " Over Time by Quantile Group"),
#          x = "Fiscal Year",
#          y = paste0("Average Value of ", var)) +
#     theme(plot.title = element_text(size = 18, hjust = .5, face = "bold", margin = margin(.1, .1, 10, .1)),
#           plot.subtitle = element_text(hjust = .5, face = "italic"),
...
if(any(table(aggregated_data$quantile_group_labels) > 0)) {
  ...
  ggplot(aggregated_data, aes(x = fiscal_year, y = mean_change, fill = quantile_group_labels)) +
    geom_col(position = "dodge") +
    facet_wrap(~quantile_group_labels, scales = "free_y") + 
    scale_x_continuous(breaks = seq(2012, 2022, by = 1), limits = c(2012, 2022)) +
    scale_y_continuous(labels = scales::comma) +
    viridis::scale_fill_viridis(discrete = TRUE, option = "mako", end = .94) +
    theme_bw()
} else {
  # Handle the situation or create an alternative output
...
endowment_data_wide <- endowment_data %>%
  ...
  mutate(across(where(is.list), ~sapply(., as.numeric)))


# Check for list columns specifically and handle them
...
```{r}
plot_combo <- function(var1, var2, data) {
  if(is.list(data[[var1]])) {
    data[[var1]] <- sapply(data[[var1]], function(x) mean(as.numeric(unlist(x)), na.rm = TRUE))
  } else {
    data[[var1]] <- as.numeric(data[[var1]])
  }
  
  if(is.list(data[[var2]])) {
    data[[var2]] <- sapply(data[[var2]], function(x) mean(as.numeric(unlist(x)), na.rm = TRUE))
  } else {
  ...
  quantile_var <- ifelse(diff(var1_range) > diff(var2_range), var1, var2)

  data %>%
    group_by(EIN) %>%
    mutate(maxval = max(!!rlang::sym(quantile_var), na.rm = TRUE)) %>%
    ungroup() %>%
    mutate(tile = ntile(maxval, 4)) %>%
    group_by(EIN) %>%
    mutate(tile = sample(tile, 1)) %>%
    filter(!is.na(tile)) %>%
    ggplot(aes(x = !!rlang::sym(var1), y = !!rlang::sym(var2), color = EIN)) +
    geom_point(alpha = .9) +
    facet_wrap(~tile, scales = "free") +
    viridis::scale_color_viridis(discrete = TRUE, option = "rocket", end = .9) +
    theme_bw() +
