...
## meaghan figure out what sp500 is/replace it properly, cannot run rest of file without it
...
inv_consist_true <- inv_consist %>% 
  filter(consistent == TRUE)

inv_consist_false <- inv_consist %>% 
  filter(consistent == FALSE)

mean(inv_consist_true$rank)
mean(inv_consist_false$rank)
...
consist_plot
ggsave("images/consist_plot.png", consist_plot)
...
inv_consist %>% 
  ...
  save_kable("images/consistent_changes.html")
...
inv_consist %>% 
  ...
  save_kable("images/inconsistent_changes.html")
