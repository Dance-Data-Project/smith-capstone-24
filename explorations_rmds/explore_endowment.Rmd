...
plot_variable_bar <- function(var) {
  ...
  data <- endowment_data %>%
      ...
      group_by(EIN) %>%
      mutate(quartile_group = max(quartile_group)) %>%
      # Ensure capitalization of quartile labels
  ...
  # Plotting conditionally on data availability
...
companies_by_quartile <- function(data, var) {
  # Wrap the potentially problematic code in a tryCatch block
  tryCatch({
    # Get EINs with at least one observation of the variable
...
list_companies_by_quartile_list <- function(data, var) {
  ...
  # Return a list of names grouped by quartile
