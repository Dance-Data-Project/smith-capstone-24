...
add <- add %>%s n
  mutate(state = case_when(
    !is.na(business_state_cd) ~ business_state_cd,
    is.na(business_state_cd) & !is.na(books_state_cd) ~ books_state_cd,
    is.na(business_state_cd) & is.na(books_state_cd) ~ books_state
  )) 
...
state_pop_link <- "https://www2.census.gov/programs-surveys/popest/datasets/2020-2023/state/totals/NST-EST2023-ALLDATA.csv"
...
if(!exists(here('data', 'stay_at_home_bu.RDS'))) {
  stay_at_home <- read_excel(here("data", "bu_data.xlsx"), sheet ='Stay at Home') %>%
...
if(!exists(here('data', 'state_of_emergency_bu.RDS'))) {
  state_of_emergency <- read_excel(here("data", "bu_data.xlsx"), sheet ='State of Emergency') %>%
  select(state = `State Abbreviation`,
         emergency_start = `State of emergency issued`, 
         emergency_end = `State of emergency lifted`
   ) %>%
  mutate(across(c(emergency_start,emergency_end),~ as_date(., origin="1899-12-31"))) %>%
  filter(!is.na(state)) %>%
        # if dates are input as 0, excel codes these as 1899-12-31; remove these
...
file_to_check <- "ballet_990_released_20230208/202001979349303705_public.xml"
file_to_check <- "ballet_990_released_20230208/201902389349200640_public.xml"
file_to_check <-  "ballet_990_released_20230208/201503459349300245_public.xml"
file_to_check <- "ballet_990_released_20230208/201543209349202514_public.xml"
file_to_check <- "ballet_990_released_20230208/202101969349301610_public.xml" 
