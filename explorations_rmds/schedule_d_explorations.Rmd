...
```{r}
# COMPANIES THAT NEVER REPORTED ENDOWMENT
all_eins <- filed_schd_d %>%
  select(EIN) %>%
  distinct()

# EINs that reported endowments
reported_eins <- filed_schd_d %>%
  filter(Filled_Sch_D == "Yes") %>%
  select(EIN) %>%
  distinct()

# Finding EINs that did NOT report endowments
never_reported_eins <- setdiff(all_eins, reported_eins)

# Joining with names to get organization names
never_reported_eins <- never_reported_eins %>%
  left_join(names, by = "EIN") %>%
  select(EIN, organization_name) %>%
  mutate(num_times_fil_sche_D = 0)  # Adding a column to show no filings
...
never_reported_eins %>%
  ...
  kableExtra::footnote(general = "This table lists EINs that have never reported an endowment.")
...
# Counting the number of companies that never reported an endowment
num_never_reported <- nrow(never_reported_eins)

# Print the count
print(paste("Number of companies that never reported an endowment:", num_never_reported))
```
...
filed_schd_d %>%
  ...
  make_table(title = "Endowment Reporting Gaps", col_names = c("EIN", "Organization Name", "Gap Length (Days)", "Gap Start Date", "Gap End Date"))
```
...
filed_schd_d %>%
  ...
  left_join(names, by = "EIN") %>%
  select(EIN, organization_name, gap, before, TaxPeriodBeginDt) %>%
  make_table(title = "990 Filing Gaps for EINs\nwhich once reported an Endowment", col_names = c("EIN", "Organization Name", "Gap Length (Days)", "Gap Start Date", "Gap End Date"))
```
...
miscalculation_table <- test_diff %>%
  filter(same_end == FALSE) %>%
  left_join(names, by = "EIN") %>%
  select(fiscal_year, organization_name) %>%
  arrange(fiscal_year, organization_name) %>%
  rename(Year = fiscal_year, Company = organization_name) %>%
  distinct()

kable(miscalculation_table, format = "html", table.attr = "style='width:100%'") %>%
