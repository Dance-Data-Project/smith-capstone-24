...
changes <- bookvalues %>% 
  ...
  select(EIN, organization_name, year, BookValueSum, change) %>% 
...
all_changes_pct <- ggplot(changes, aes(x = year, y = changes$change, color =  as.factor(organization_name))) +
...
ranking<-changes %>%
  arrange(desc(change_abs)) %>%  # percent_change ????
