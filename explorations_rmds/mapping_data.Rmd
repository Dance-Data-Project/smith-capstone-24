...
company_sample <- read_csv(here("data","top_companies.csv"))
...
mappable_data <- company_sample %>% 
  left_join(company_balances, by = "Company") %>% 
  mutate(mean_begin = round(mean_begin, digits = 0), mean_end = round(mean_end, digits = 0)) %>% 
  rename("Average Start of Year Balance" = mean_begin, "Average End of Year Balance" = mean_end)

write_csv(mappable_data, here("data", "top10s_data.csv"))
