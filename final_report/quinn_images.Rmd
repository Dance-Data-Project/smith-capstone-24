...
The percent change is then simply $\text{Relative Change} \times 100$.

For a relative change of value $R$, we can interpret it by considering
the formula

$$\text{End Value} = (R+1) \times \text{Start Value}.$$

A couple of examples include: \* If the percent change is $-50\%$, the
current year value is half of that in the previous year. \* If the
percent change is $100\%$, the current year value is twice that in the
previous year. \* If the percent change is $-100\%$, the current year
value is zero.
...
We can compute this for the S&P 500 for the same time interval. Here, we
use mean value of the S&P 500 to the previous week compared to the
beginning and end dates of each company's fiscal year.

To account for withdrawals and contributions, we add back withdrawals
and subtract contributions, so we calculate the annual growth rate as
...
This adjustment is important because we want to see how much of the
change from the beginning of year balance to the end of year balance is
due to the investments, not, for example, a large contribution.
...
end_sp %>%
  ...
  ylim(-.1,.25)
ggsave(here('final_report/images/annual-growth.pdf'))
...
percent_change <- end_sp %>%
  ...
  mutate(pct_change_sp = (sp500_end - sp500_begin) / sp500_begin) %>%
  select(contains("pct_change"), organization_name, fiscal_year) %>%
  mutate(pct_change_adjusted = round((100 * pct_change_adjusted), digits = 2), pct_change_sp = round((100 * pct_change_sp), digits = 2)) %>% 
  rename("Relative Percent Change (%)" = pct_change_adjusted, "S&P 500 Percent Change (%)" = pct_change_sp, "Organization" = organization_name, "Fiscal Year" = fiscal_year)
  #pivot_longer(c(contains('pct_change')))


# DONT RUN MORE THAN ONCE
percent_change <- percent_change[,c(3, 4, 1, 2)]

library(kableExtra)
percent_change %>% 
  kbl(caption = "Relative and S&P 500 Percent Change for Ballet Companies") %>% 
  kable_material(c("striped", "hover")) %>% 
  save_kable("images/pct_change.html")

#webshot::webshot(url = NULL, file = "pct_change.pdf")
...
Now, we note that we can compute the withdrawals using other variables
reported in the 990:
...
-->
```
We visualize the compound growth rates for all companies in Figure
\ref{fig:compound-growth}. Notably, San Francisco Ballet, Joffrey
Ballet, and
...
cagr_2017_2023 <- end_sp %>%
    filter(fiscal_year <= 2023 & fiscal_year >= 2017) %>%
  group_by(organization_name) %>%
...
cagr_2019_2023 <- end_sp %>%
    filter(fiscal_year>= 2019 & fiscal_year <= 2023) %>%
  group_by(organization_name) %>%
  mutate(minyear = min(fiscal_year),
         maxyear = max(fiscal_year)) %>%
  mutate(across(c(AdministrativeExpensesAmt, OtherExpendituresAmt,
...
cagr_2021_2023 <- end_sp %>%
    filter(fiscal_year>= 2021 & fiscal_year <= 2023) %>%
  group_by(organization_name) %>%
  mutate(minyear = min(fiscal_year),
         maxyear = max(fiscal_year)) %>%
  mutate(across(c(AdministrativeExpensesAmt, OtherExpendituresAmt,
...
plt1 <- cagr_2017_2023 %>%
  plot_cagr(title = "2017 to 2023") +
  theme(legend.position="none")



plt2 <- cagr_2019_2023 %>%
  plot_cagr(title = "2019 to 2023")+
  theme(legend.position="none")
...
#write_csv(cagr_2021_2023, "cagr_2021_2023.csv")

```

We see immediately in Figure \ref{fig:compound-growth} that some
companies have a compound growth rate is indistinguishable from zero.
This includes:

-   Oregon Ballet: reported no investment earnings/losses for any year
-   Harlem Ballet: investment earnings/gains were extremely small
    ($\pm 13$ dollars)
-   First State Ballet Theatre: reported no investment earnings/losses
    for any year
-   Eugene Ballet: only reported investment earnings/losses for 2020 and
    2021
-   American Repertory Ballet: reported no investment earnings/losses
    for any year

We see in Figure \ref{fig:annual-growth-endowment} that each of these
companies annual growth rates are essentially zero when accounting for
...
plot_ranks_by_consistency <- function(var1, var2, data) {
   ...
   plt <- data %>%
     filter(fiscal_year > 2010 & fiscal_year < 2023) %>%
    group_by(fiscal_year) %>%
    mutate("{var1}_rank" := rank(-!!sym(var1)), na.last = "keep") %>%
    mutate("{var2}_rank"  := rank(-!!sym(var2)),  na.last = "keep")  %>%
...
endowment_designated <- 
  ggplot(data=all_category, 
         aes(x=fiscal_year, y= BoardDesignatedBalanceEOYPct, fill=organization_name)) + 
  geom_bar(stat="identity", width=.5, fill="tomato3") + 
  labs(title="Endowment Performance Bar Chart", 
       subtitle="Board designated, quasi permanent, and term endowments", 
       caption="source: DDP 2014-2022 Data") + 
...
endowment_term <- 
  ggplot(data=all_category, 
         aes(x=fiscal_year, y= TermEndowmentBalanceEOYPct, fill=organization_name)) + 
  geom_bar(stat="identity", width=.5, fill="tomato3") + 
  labs(title="Endowment Performance Bar Chart", 
       subtitle="Board designated, quasi permanent, and term endowments", 
       caption="source: DDP 2014-2022 Data") + 
...
endowment_permanent <- 
  ggplot(data=all_category, 
         aes(x=fiscal_year, y= PrmnntEndowmentBalanceEOYPct, fill=organization_name)) + 
  geom_bar(stat="identity", width=.5, fill="tomato3") + 
  labs(title="Endowment Performance Bar Chart", 
       subtitle="Board designated, quasi permanent, and term endowments", 
       caption="source: DDP 2014-2022 Data") + 
...
all_category$PrmnntEndowmentBalanceEOYPct <- as.numeric(as.character(all_category$PrmnntEndowmentBalanceEOYPct))


permanent_summaries<-
  all_category |> 
  filter(!is.na(PrmnntEndowmentBalanceEOYPct)) |>
  select(organization_name, PrmnntEndowmentBalanceEOYPct, fiscal_year) |>
  group_by(organization_name) |>
  mutate(
    group_mean=mean(PrmnntEndowmentBalanceEOYPct),
    group_sd = sd(PrmnntEndowmentBalanceEOYPct),
    group_min = min(PrmnntEndowmentBalanceEOYPct),
    group_max = max(PrmnntEndowmentBalanceEOYPct))
...
endowment_data_wide$BeginningYearBalanceAmt <- as.numeric(as.character(endowment_data_wide$BeginningYearBalanceAmt))
endowment_data_wide$ContributionsAmt <- as.numeric(as.character(endowment_data_wide$ContributionsAmt))
endowment_data_wide$OtherExpendituresAmt <- as.numeric(as.character(endowment_data_wide$OtherExpendituresAmt))
endowment_data_wide$AdministrativeExpensesAmt <- as.numeric(as.character(endowment_data_wide$AdministrativeExpensesAmt))
endowment_data_wide$GrantsOrScholarshipsAmt <- as.numeric(as.character(endowment_data_wide$GrantsOrScholarshipsAmt))
endowment_data_wide$InvestmentEarningsOrLossesAmt <- as.numeric(as.character(endowment_data_wide$InvestmentEarningsOrLossesAmt))
endowment_data_wide$EndYearBalanceAmt <- as.numeric(as.character(endowment_data_wide$EndYearBalanceAmt))
...
prop_balanced_ranked_higher <- endowment_data_wide %>%
     filter(fiscal_year > 2010 & fiscal_year < 2024) %>%
    #group_by(fiscal_year) %>%
   # arrange(var1) %>%
    mutate("{var1}_rank" := rank(-!!sym(var1)), na.last = "keep") %>%
#    arrange(var2) %>%
  ...
  select(fiscal_year, prop_positive, organization_name, sum_pos, sum_neg, sum_zero)  %>%
  distinct()
...
df <- prop_balanced_ranked_higher %>%
  select(contains("sum"), organization_name, prop_positive, fiscal_year) %>%
  ...
  ggplot(df, aes(y = fct_reorder(organization_name,prop_positive),x=  prop, fill=name)) +
...
```{r}
#New plot
df_new <- endowment_data_wide %>% 
  mutate(name = case_when(
    as.numeric(unlist(BeginningYearBalanceAmt)) > as.numeric(unlist(ContributionsAmt)) ~ "Beginning of Year Balance\nIs Higher",
    as.numeric(unlist(BeginningYearBalanceAmt)) < as.numeric(unlist(ContributionsAmt)) ~ "Beginning of Year Balance\nIs Lower",
    as.numeric(unlist(BeginningYearBalanceAmt)) == as.numeric(unlist(ContributionsAmt)) ~"The Same"
  )) 
...
ggplot(df_group, aes(x=as.factor(fiscal_year), y= n, color = as.factor(name), fill = as.factor(name))) +
  geom_bar(stat = "identity", position = "dodge")+
  geom_text(aes(label= n), vjust=-0.3, size=3.5)+
  theme_bw() + 
  labs(title = "Number of Ballet Companies Where Beginning of Year Balance Is Higher, Lower, or Equal to Contribution",
       y = "Number of Ballet Companies",
       x = "Fiscal Year") + 
  theme(axis.text.x = element_text(size =25),
        ...
        strip.text=element_text(size = 16, color = "white"),
        plot.title=element_text(hjust = .5, face="bold", size = 35))
```

```{r}
df_name <- df_new %>% 
  select(EIN, organization_name, fiscal_year, name) %>% 
  filter(!is.na(name)) %>% 
  write.csv(here('data', 'contribution_vs_BOY.csv'))
```
...
balance_ranks <- endowment_data_wide %>%
    filter(fiscal_year >=2011 & fiscal_year <=2024) %>%
    group_by(fiscal_year) %>%
...
balance_ranks <- balance_ranks %>% 
 ...
 labels_all <- balance_ranks %>%
   ...
   mutate(minyear = min(fiscal_year,na.rm=TRUE),
          maxyear = max(fiscal_year, na.rm=TRUE)) %>%
   mutate(organization_name = ifelse(maxyear == 2023, organization_name, NA)) %>%
 ...
 balance_ranks %>%
   ...
   mutate(minyear = min(fiscal_year,na.rm=TRUE)) %>%
    ...
    scale_x_continuous(breaks=2012:2022,limits=c(2012,2022)) +
   ...
   geom_text(aes(x=2023, label = organization_name, y  = rank), 
...
-   For total employee compensation -
    `CYSalariesCompEmpBnftPaidAmt: Salaries, other compensation, employee benefits (Part IX, column (A), lines 5–10)`.
-   For top employee compensation - Schedule J, looking at total
    compensation
